...
export function DossierViewer() {
  ...
  useEffect(() => {
    ...
    if (saved === "zh" || saved === "en") {
      setLang(saved);
      if (saved === "en") setSelectedPlatformFilter("All");
    }
  }, []);
  ...
  function toggleLanguage() {
    ...
    setLang(next);
    setSelectedPlatformFilter(next === "zh" ? "全部" : "All");
  ...
  return (
    <div className="w-full">
      ...
      {!isCustomMode ? (
        /* Platform Filters & Hot Dossiers Tabs */
        <div className="mt-2">
          ...
          {/* Case Cards Carousel */}
          <div className="flex gap-3 overflow-x-auto pb-3 pt-1 no-scrollbar justify-start md:justify-center">
            {filteredCases.map((rawC) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "flex items-center gap-3 px-4 py-2.5 rounded-[var(--radius-lg)] border text-left transition-all shrink-0 max-w-[280px] cursor-pointer",
                    active
                ...
                >
                  ...
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-wider text-pause flex items-center gap-1">
                      <span>{c.platformIcon}</span>
                      <span>{c.platform.split(" ")[0]}</span>
                    </p>
                    <p className="text-xs font-bold mt-0.5 line-clamp-1 text-inherit">
                      {c.title}
                    </p>
                  </div>
                </button>
      ...
      <div
        ...
        className="w-full max-w-[840px] mx-auto mt-6 bg-ink text-canvas rounded-[var(--radius-xl)] p-6 md:p-8 border border-line-strong shadow-2xl overflow-hidden box-border"
      >
        {/* Dossier Header */}
...
with open("src/features/dossier/DossierViewer.tsx", "w", encoding="utf-8") as f:
  f.write(code)

print("Updated DossierViewer.tsx with full dynamic case localization successfully")
